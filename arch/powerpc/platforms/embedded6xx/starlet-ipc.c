...
static int starlet_ipc_of_probe(struct platform_device *odev)

{
	...
	error = of_address_to_resource(odev->dev.of_node, 1, &mem[1]);
	if (error) {
...
static int starlet_ipc_of_remove(struct platform_device *odev)
{
	return starlet_ipc_do_remove(&odev->dev);
}

static void starlet_ipc_of_shutdown(struct platform_device *odev)
{
	starlet_ipc_do_shutdown(&odev->dev);
}
...
static struct platform_driver starlet_ipc_of_driver = {
	.driver = {
		.name = DRV_MODULE_NAME,
		.owner = THIS_MODULE,
		.of_match_table = starlet_ipc_of_match,
	},
...
static int __init starlet_ipc_init_module(void)
{
	...
	return platform_driver_register(&starlet_ipc_of_driver);
}

static void __exit starlet_ipc_exit_module(void)
{
	platform_driver_unregister(&starlet_ipc_of_driver);
}
